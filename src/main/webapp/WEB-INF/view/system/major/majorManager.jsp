...
<div id="majorBar" style="display: none;">
...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'form', 'table', 'upload'], function () {
        ...
        tableIns = table.render({
            elem: '#majorTable'   //渲染的目标对象
            , url: '${pageContext.request.contextPath}/major/loadAllMajor.action' //数据接口
            , title: '数据表'//数据导出来的标题
            , toolbar: "#majorToolBar"   //表格的工具条
            , height: 'full-210'
            ...
            , cols: [[   //列表数据
                ...
                , {fixed: 'right', title: '操作', toolbar: '#majorBar', align: 'center'}
            ]],
...
//            alert(params);
            tableIns.reload({
                url: "${pageContext.request.contextPath}/major/loadAllMajor.action?" + params,
                page: {curr: 1}
            })
        });
        ...
        table.on('tool(majorTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.name + '】这个专业么？', function (index) {
                    //向服务端发送删除指令
                    $.post("${pageContext.request.contextPath}/major/deleteMajor.action", {id: data.id}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
        ...
        function openAddMajor() {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    //清空表单数据
                    $("#dataFrm")[0].reset();
                    url = "${pageContext.request.contextPath}/major/addMajor.action";
                }
            });
        }
        ...
        function openUpdateMajor(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    url = "${pageContext.request.contextPath}/major/updateMajor.action";
                }
            });
        }
        ...
        function deleteBatch() {
            ...
            layer.confirm('真的要删除这些专业么？', function (index) {
                //向服务端发送删除指令
                $.post("${pageContext.request.contextPath}/major/deleteBatchMajor.action", params, function (res) {
                    layer.msg(res.msg);
                    //刷新数据表格
                    tableIns.reload();
                })
