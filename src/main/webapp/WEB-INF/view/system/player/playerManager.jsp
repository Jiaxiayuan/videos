...
<!-- 数据表格开始 -->
<table class="layui-hide" id="playerTable" lay-filter="playerTable"></table>
        ...
        <div class="layui-form-item">
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    ...
                    <input type="text" name="name" lay-verify="required" placeholder="请输入选手姓名" autocomplete="off" class="layui-input">
                </div>
            </div>
        </div>
        <div class="layui-form-item">
...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'form', 'table'], function () {
        ...
        tableIns = table.render({
            elem: '#playerTable'   //渲染的目标对象
            , url: '${pageContext.request.contextPath}/player/loadAllPlayer.action' //数据接口
            , title: '选手数据表'//数据导出来的标题
            , toolbar: "#playerToolBar"   //表格的工具条
            , height: '600'
            ...
            , cols: [[   //列表数据
                ...
                , {field: 'qq', title: 'QQ号码', align: 'center'}
                , {field: 'color', title: '颜色', align: 'center'}
                , {fixed: 'right', title: '操作', toolbar: '#playerBar', align: 'center'}
            ]],
        ...
        $("#doSearch").click(function () {
            ...
            //alert(params);
            tableIns.reload({
                url: "${pageContext.request.contextPath}/player/loadAllPlayer.action?" + params,
                page:{curr:1}
            })
        });
        ...
        table.on('tool(playerTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.name + '】这个选手吗', function (index) {
                    //向服务端发送删除指令
                    $.post("${pageContext.request.contextPath}/player/deletePlayer.action", {id: data.id}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
        ...
        function openAddPlayer() {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    //清空表单数据
                    $("#dataFrm")[0].reset();
                    url = "${pageContext.request.contextPath}/player/addPlayer.action";
                }
            });
        }

        //打开修改页面
        function openUpdatePlayer(data) {
            mainIndex = layer.open({
                type: 1,
                title: '修改选手',
        ...
        function deleteBatch() {
            ...
            layer.confirm('真的要删除这些用户么？', function (index) {
                //向服务端发送删除指令
                $.post("${pageContext.request.contextPath}/player/deleteBatchPlayer.action",params, function (res) {
                    layer.msg(res.msg);
                    //刷新数据表格
                    tableIns.reload();
                })
